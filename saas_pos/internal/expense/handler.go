...
func HandleCreate(c *fiber.Ctx) error {
	...
	exp, err := Create(claims.TenantID, input)
	if err != nil {
		return c.Status(400).JSON(fiber.Map{"error": err.Error()})
	}
	return c.Status(201).JSON(fiber.Map{"data": exp})
}

// GET /api/tenant/expenses?search=&from=&to=&page=&limit=
func HandleList(c *fiber.Ctx) error {
	...
	result, err := List(claims.TenantID, search, from, to, page, limit)
	if err != nil {
...
func HandleUpdate(c *fiber.Ctx) error {
	...
	exp, err := Update(claims.TenantID, id, input)
	if err != nil {
		return c.Status(400).JSON(fiber.Map{"error": err.Error()})
	}
...
// GET /api/tenant/expenses/sum?from=&to=
