...
func HandleCreate(c *fiber.Ctx) error {
	...
	loss, err := Create(tenantID, input)
	if err != nil {
		return response.Error(c, fiber.StatusBadRequest, err.Error())
	}
	return c.Status(fiber.StatusCreated).JSON(fiber.Map{"data": loss})
}

// GET /api/tenant/losses?search=&from=&to=&page=&limit=
