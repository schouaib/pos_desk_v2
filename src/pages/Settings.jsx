...
export default function Settings({ path }) {
  ...
  // Load printers and saved printer preferences
  useEffect(() => {
    ...
    <Layout currentPath={path}>
      ...
      <form onSubmit={handleSave} class="space-y-4">
        ...
        {window.__TAURI_INTERNALS__ && (
          <Section
            title={t('printerSettings') || 'Printer Settings'}
            icon={
              <svg xmlns="http://www.w3.org/2000/svg" class="w-4.5 h-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
          ...
          >
            {printersList.length === 0 ? (
              <p class="text-sm text-base-content/50">{t('noPrintersFound') || 'No printers detected'}</p>
            ) : (
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <label class="form-control">
