...
export default function Products({ path }) {
  ...
  const canViewPrixAchat = hasPerm('products', 'view_prix_achat')
  const canViewQty     = hasPerm('products', 'view_qty')
  const canViewPV1     = hasPerm('products', 'view_pv1')
  const canViewPV2     = hasPerm('products', 'view_pv2')
  const canViewPV3     = hasPerm('products', 'view_pv3')
  ...
  const [actionMenu, setActionMenu] = useState(null) // { product, x, y }
  ...
  const [batchDeleteId, setBatchDeleteId] = useState(null)
  ...
  useEffect(() => {
    function onKey(e) {
      ...
      if ((e.ctrlKey || e.metaKey) && e.key === 'ArrowRight' && !quickMode) {
        e.preventDefault()
        setTab(prev => Math.min(prev + 1, 2))
      }
      // Ctrl+ArrowLeft → prev tab
      if ((e.ctrlKey || e.metaKey) && e.key === 'ArrowLeft' && !quickMode) {
        e.preventDefault()
  ...
  async function handleSubmit(e) {
    ...
    // Inline validation — collect all field errors
    const errs = {}
    if (!form.name.trim()) errs.name = t('productName') + ' ' + t('required')
    const defaultPriceKey = defaultSalePrice === 2 ? 'prix_vente_2' : defaultSalePrice === 3 ? 'prix_vente_3' : 'prix_vente_1'
    if (form[defaultPriceKey] <= 0 && !form.is_service) errs.prix_vente_1 = (defaultSalePrice === 2 ? t('prixVente2') : defaultSalePrice === 3 ? t('prixVente3') : t('prixVente1')) + ' ' + t('required')
    setFieldErrors(errs)
    if (Object.keys(errs).length > 0) {
      setError(Object.values(errs)[0])
      if (errs.name) setTab(0)
      else if (errs.prix_vente_1) setTab(quickMode ? 0 : 1)
      return
    }
    setError('')
    setLoading(true)
  ...
  async function confirmDuplicate() {
    if (!dupTarget) return
    try {
      await api.duplicateProduct(dupTarget.id)
      setDupTarget(null)
  ...
  function deleteVariant(id) {
    ...
    openModal('delete-variant-modal')
  }
  async function confirmDeleteVariant() {
    if (!deleteVariantId) return
    try { await api.deleteVariant(deleteVariantId); setVariantItems(await api.listVariants(variantTarget.id)); load() } catch {}
    setDeleteVariantId(null)
    closeModal('delete-variant-modal')
  }
  ...
  function deleteDiscount(id) {
    ...
    openModal('delete-discount-modal')
  }
  async function confirmDeleteDiscount() {
    if (!deleteDiscountId) return
    try { await api.deleteDiscount(deleteDiscountId); setDiscountItems(await api.listProductDiscounts(discountTarget.id)) } catch {}
    setDeleteDiscountId(null)
    closeModal('delete-discount-modal')
  }
  ...
  function deleteBatch(id) {
    setBatchDeleteId(id)
    openModal('delete-batch-modal')
  }
  async function confirmDeleteBatch() {
    try {
      await api.deleteBatch(batchDeleteId)
      const r = await api.listProductBatches(batchTarget.id, { page: 1, limit: 50 })
      setBatchItems(r.items || [])
      load()
    } catch {}
    setBatchDeleteId(null)
    closeModal('delete-batch-modal')
  }
    ...
    <Layout currentPath={path}>
        ...
        <table class="table table-sm w-full">
          <thead class="bg-base-200/60 sticky top-0 z-10">
            <tr>
              ...
              <th class="px-3 py-2.5 text-xs font-semibold uppercase tracking-wide text-base-content/70">{t('category')}</th>
              {canViewQty && <th class="px-3 py-2.5 text-xs font-semibold uppercase tracking-wide text-base-content/70 text-end">{t('qtyAvailable')}</th>}
              {canViewPrixAchat && <th class="px-3 py-2.5 text-xs font-semibold uppercase tracking-wide text-base-content/70 text-end">{t('prixAchat')}</th>}
              {(defaultSalePrice === 1 ? canViewPV1 : defaultSalePrice === 2 ? canViewPV2 : canViewPV3) && (
                <th class="px-3 py-2.5 text-xs font-semibold uppercase tracking-wide text-base-content/70 text-end">{defaultSalePrice === 2 ? t('prixVente2') : defaultSalePrice === 3 ? t('prixVente3') : t('prixVente1')}</th>
              )}
              <th class="px-3 py-2.5 text-xs font-semibold uppercase tracking-wide text-base-content/70 text-end w-36">{t('actions')}</th>
            </tr>
          </thead>
          <tbody>
            {items.map((p, _idx) => (
              <tr key={p.id} class="border-b border-base-200/50 hover:bg-primary/[0.03] transition-colors group">
                  ...
                  <div class="flex items-center gap-3">
                    {p.image_url
                      ...
                      : <div class="w-9 h-9 rounded-lg bg-base-200/80 flex items-center justify-center shrink-0">
                          <span class="text-xs font-bold text-base-content/50">{p.name?.charAt(0)?.toUpperCase()}</span>
                        </div>
                    }
                    <div class="min-w-0">
                      ...
                      <div class="flex items-center gap-1.5 mt-0.5 flex-wrap">
                        {p.barcodes?.length > 0 && (
                          <span class="text-xs font-mono text-base-content/70">{p.barcodes[0]}</span>
                        )}
                        {p.ref && <span class="text-xs text-base-content/70">ref: {p.ref}</span>}
                ...
                {/* Category + brand + unit */}
                <td class="px-3 py-2.5">
                  <div class="text-sm">{catMap.get(p.category_id) || <span class="text-base-content/50">—</span>}</div>
                  ...
                  <td class="px-3 py-2.5 text-end">
                    {p.is_service
                      ...
                      : <div class="flex flex-col items-end">
                          ...
                          {p.has_variants && <span class="text-xs text-primary/50">{t('variants')}</span>}
                        </div>
                    }
                  </td>
                )}
                  ...
                  <div class="flex items-center justify-end gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
                    {canEdit && (
                      <div class="tooltip tooltip-left" data-tip={t('edit')}>
                        <button class="btn btn-sm btn-ghost btn-square" onClick={() => openEdit(p)}>
                    ...
                    {canEdit && (
                      <div class="tooltip tooltip-left" data-tip={t('printLabel')}>
                        <button class="btn btn-sm btn-ghost btn-square text-info" onClick={() => { setPrintTarget(p); setTimeout(() => openModal(PRINT_MODAL_ID), 100) }}>
                          <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                    ...
                    {/* More actions button */}
                    <button class="btn btn-sm btn-ghost btn-square" onClick={(e) => {
                      const rect = e.currentTarget.getBoundingClientRect()
                      const menuH = 280 // approximate menu height
                      // If not enough space above, open below
                      const openAbove = rect.top > menuH
                      const y = openAbove ? rect.top : rect.bottom
                      setActionMenu(actionMenu?.product?.id === p.id ? null : { product: p, x: rect.left, y, above: openAbove })
                    }}>
                      <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
      ...
      <Modal id="product-modal" title={editing ? t('editProduct') : t('newProductTitle')} size="2xl">
        ...
        <div class="flex items-center gap-2 mb-4">
          <div class="flex items-center bg-base-200/60 rounded-lg p-0.5 shrink-0">
            <button type="button" onClick={() => setQuickMode(false)}
              class={`px-2.5 py-1.5 rounded-md text-xs font-semibold transition-all ${!quickMode ? 'bg-primary text-primary-content shadow-sm' : 'text-base-content/70 hover:text-base-content/80'}`}>
              {t('advanced')}
            </button>
          ...
          {!quickMode && (
            <div class="flex items-center flex-1 bg-base-200/60 rounded-lg p-1 gap-1">
              {[
                ...
                <button
                  ...
                  class={`flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-lg text-xs font-semibold transition-all
                    ${tab === i ? 'bg-primary text-primary-content shadow-sm' : tab > i ? 'text-success bg-success/8' : 'text-base-content/70 hover:text-base-content/80 hover:bg-base-300/50'}`}
                >
                  {tab > i
        ...
        <p class="text-xs text-base-content/50 mb-3">Ctrl+Enter {t('save')} &bull; Ctrl+←→ {t('navigate')}</p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          {quickMode && (
            <div class="space-y-4">
              ...
              <label class="form-control w-full">
                ...
                <input class={`input input-bordered ${fieldErrors.name ? 'input-error' : ''}`} value={form.name}
                  onInput={(e) => { setForm({ ...form, name: e.target.value }); setFieldErrors(f => ({...f, name: ''})) }}
                  placeholder={t('productName')} autoFocus />
                {fieldErrors.name && <span class="text-xs text-error mt-1">{fieldErrors.name}</span>}
              </label>
              ...
              <div class="form-control w-full">
                ...
                <div class="join w-full">
                  <input class="input input-bordered join-item flex-1" value={barcodeInput}
                    placeholder={t('addBarcode')}
                    onInput={(e) => setBarcodeInput(e.target.value)}
                    onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); addBarcode() } }} />
                  <button type="button" class="btn btn-outline join-item" onClick={addBarcode}>+</button>
                  <button type="button" class="btn btn-secondary join-item gap-1.5" onClick={async () => {
                    try {
                ...
                {form.barcodes.length > 0 && (
                  <div class="mt-2">
                    <div class="flex items-center justify-between mb-1">
                      <span class="text-xs text-base-content/60">{form.barcodes.length} {t('barcodes')}</span>
                      {form.barcodes.length > 1 && (
                        <button type="button" class="text-xs text-error hover:underline" onClick={() => setForm({...form, barcodes: []})}>{t('clearAll')}</button>
                      )}
                    </div>
                    <div class="max-h-24 overflow-y-auto rounded-xl border border-base-200 bg-base-200/30 divide-y divide-base-200">
                      {form.barcodes.map((b) => (
                        <div key={b} class="flex items-center justify-between px-3 py-1.5 hover:bg-base-200/60 group">
                          <span class="text-sm font-mono text-base-content/80 truncate">{b}</span>
                          <button type="button" class="text-error/40 group-hover:text-error text-lg leading-none px-1" onClick={() => removeBarcode(b)}>×</button>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <div class="bg-base-200/40 rounded-xl p-4 space-y-3">
                <p class="text-xs font-bold uppercase tracking-wider text-base-content/60">{t('pricing')}</p>
                <div class="grid grid-cols-2 gap-3">
                  <label class="form-control">
                    <div class="label py-0.5">
                      <span class="label-text text-sm">{t('prixAchat')}</span>
                    </div>
                    <input type="number" step="any" min="0" class="input input-bordered"
                      value={form.prix_achat} onInput={(e) => setForm({ ...form, prix_achat: parseFloat(e.target.value) || 0 })} />
                  </label>
                  <label class="form-control">
                    <div class="label py-0.5">
                      <span class="label-text text-sm font-semibold">{defaultSalePrice === 2 ? t('prixVente2') : defaultSalePrice === 3 ? t('prixVente3') : t('prixVente1')} <span class="text-error">*</span></span>
                    </div>
                    <input type="number" step="any" min="0" class={`input input-bordered ${fieldErrors.prix_vente_1 ? 'input-error' : ''}`}
                      value={defaultSalePrice === 2 ? form.prix_vente_2 : defaultSalePrice === 3 ? form.prix_vente_3 : form.prix_vente_1}
                      onInput={(e) => {
                        const v = parseFloat(e.target.value) || 0
                        const key = defaultSalePrice === 2 ? 'prix_vente_2' : defaultSalePrice === 3 ? 'prix_vente_3' : 'prix_vente_1'
                        setForm({ ...form, [key]: v })
                  ...
                  {!editing && (
                    <label class="form-control">
                      <div class="label py-0.5">
                        <span class="label-text text-sm">{t('qtyAvailable')}</span>
                      </div>
                      <input type="number" step="any" min="0" class="input input-bordered"
                        value={form.qty_available} onInput={(e) => setForm({ ...form, qty_available: parseFloat(e.target.value) || 0 })} />
                    </label>
                  )}
                  {useVAT && (
                    <label class="form-control">
                      ...
                      <div class="relative">
                        <input type="number" min="0" max="100" step="1" class="input input-bordered w-full pe-8"
                          value={form.vat} onInput={(e) => setForm({ ...form, vat: Math.min(100, Math.max(0, parseInt(e.target.value) || 0)) })} />
                        <span class="absolute end-3 top-1/2 -translate-y-1/2 text-sm text-base-content/50 pointer-events-none">%</span>
                      </div>
                    </label>
                  )}
                </div>
          ...
          {!quickMode && tab === 0 && (
            <div class="space-y-3">
              ...
              <div class="flex gap-3 items-start">
                <div class="flex-1 space-y-2">
                  <div class="flex flex-col">
                    <span class="text-xs font-semibold text-base-content/70 mb-0.5">{t('productName')} *</span>
                    <input class={`input input-bordered input-sm ${fieldErrors.name ? 'input-error' : ''}`} value={form.name} required
                      onInput={(e) => { setForm({ ...form, name: e.target.value }); setFieldErrors(f => ({...f, name: ''})) }} />
                  </div>
                  <div class="grid grid-cols-2 gap-2">
                    ...
                    <div class="flex flex-col">
                      <span class="text-xs text-base-content/70 mb-0.5">{t('abbreviation')}</span>
                ...
                <div class="shrink-0 flex flex-col items-center gap-1">
                  <div class="w-20 h-20 rounded-lg border-2 border-dashed border-base-300 flex items-center justify-center overflow-hidden bg-base-200 transition-colors hover:border-primary/40">
                    {(imagePreview || form.image_url)
                      ...
                      : <svg xmlns="http://www.w3.org/2000/svg" class="w-8 h-8 text-base-content/50" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="m2.25 15.75 5.159-5.159a2.25 2.25 0 0 1 3.182 0l5.159 5.159m-1.5-1.5 1.409-1.409a2.25 2.25 0 0 1 3.182 0l2.909 2.909M3.75 21h16.5A2.25 2.25 0 0 0 22.5 18.75V5.25A2.25 2.25 0 0 0 20.25 3H3.75A2.25 2.25 0 0 0 1.5 5.25v13.5A2.25 2.25 0 0 0 3.75 21Z" /></svg>
                    }
                  </div>
                  ...
                  {imageError && <p class="text-xs text-error">{imageError}</p>}
                </div>
              </div>

              {/* Classification */}
              <div class="bg-base-200/50 rounded-lg p-2.5">
                <p class="text-xs font-semibold text-base-content/70 uppercase tracking-wide mb-1.5">{t('categoriesPage')} / {t('brandsPage')} / {t('unitsPage')}</p>
                <div class="grid grid-cols-3 gap-2">
                  <div class="flex flex-col">
                    <span class="text-xs text-base-content/70 mb-0.5">{t('categoriesPage')}</span>
                  ...
                  <div class="flex flex-col">
                    <span class="text-xs text-base-content/70 mb-0.5">{t('brandsPage')}</span>
                  ...
                  <div class="flex flex-col">
                    <span class="text-xs text-base-content/70 mb-0.5">{t('unitsPage')}</span>
              ...
              <div class="bg-base-200/50 rounded-lg p-2.5 space-y-1.5">
                <p class="text-xs font-semibold text-base-content/70 uppercase tracking-wide">{t('barcodes')}</p>
                <div class="flex gap-1">
                ...
                {form.barcodes.length > 0 && (
                  <div>
                    <div class="flex items-center justify-between mb-1">
                      <span class="text-[10px] text-base-content/60">{form.barcodes.length} {t('barcodes')}</span>
                      {form.barcodes.length > 1 && (
                        <button type="button" class="text-[10px] text-error hover:underline" onClick={() => setForm({...form, barcodes: []})}>{t('clearAll')}</button>
                      )}
                    </div>
                    <div class="max-h-24 overflow-y-auto rounded-lg border border-base-200 bg-base-200/30">
                      {form.barcodes.map((b) => (
                        <div key={b} class="flex items-center justify-between px-2 py-0.5 hover:bg-base-200/60 group">
                          <span class="text-xs font-mono text-base-content/80 truncate">{b}</span>
                          <button type="button" class="text-error/40 group-hover:text-error text-sm leading-none px-1" onClick={() => removeBarcode(b)}>×</button>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <div class="flex flex-wrap gap-4 items-center">
                ...
                {canBatches && !form.is_service && (
                  <div class="flex items-center gap-2">
                    <span class="text-xs text-base-content/80">{t('expiryAlertDays')}:</span>
            ...
            <div class="space-y-5">
              ...
              <div class="bg-base-200/50 rounded-xl p-4">
                <div class="flex items-center gap-3">
                  ...
                  <span class="text-xs font-semibold text-base-content/80 uppercase tracking-wide">{t('vatRate')}</span>
                  <div class="relative">
              ...
              {/* Sale prices */}
              <div class="bg-base-200/50 rounded-xl p-4">
                <p class="text-xs font-semibold text-base-content/70 uppercase tracking-wide mb-3">{t('pricing')}</p>
                <div class="space-y-3">
                  {[
                    { label: t('prixVente1'), key: 'prix_vente_1', color: 'primary', vis: 'pv1' },
                    { label: t('prixVente2'), key: 'prix_vente_2', color: 'info', vis: 'pv2' },
                    { label: t('prixVente3'), key: 'prix_vente_3', color: 'accent', vis: 'pv3' },
                  ].filter(({ vis }) => visiblePrices[vis]).map(({ label, key, color }) => {
          ...
          {!quickMode && tab === 2 && (
            form.is_service
              ? <div class="flex flex-col items-center justify-center py-10 text-base-content/70">
          ...
          <div class="modal-action mt-6 border-t border-base-200 pt-4 flex items-center">
            ...
            {!quickMode && tab < 2 && (
              <button type="button" class="btn btn-sm btn-outline gap-1.5 min-w-24" onClick={() => {
                  if (tab === 0 && !form.name.trim()) { setFieldErrors({name: t('productName') + ' ' + t('required')}); setError(t('productName') + ' ' + t('required')); return }
                  setError('')
                  setFieldErrors({})
                  setTab(tab + 1)
                }}>
            ...
            <button type="button" onClick={handleSubmit} class={`btn btn-primary btn-sm min-w-28 ms-2 ${loading ? 'loading' : ''}`} disabled={loading}>
      ...
      <Modal id="duplicate-modal" title={t('duplicate')}>
        <p class="text-sm mb-4">{dupTarget?.name}</p>
        <div class="modal-action">
          <button class="btn btn-primary btn-sm" onClick={confirmDuplicate}>{t('duplicate')}</button>
          <button class="btn btn-sm btn-ghost" onClick={() => closeModal('duplicate-modal')}>{t('cancel')}</button>
        </div>
      </Modal>

      {/* Delete discount confirmation */}
      <Modal id="delete-discount-modal" title={t('deleteConfirm')}>
        <p class="text-sm mb-4">{t('discountRules')}</p>
        <div class="modal-action">
          <button class="btn btn-error btn-sm" onClick={confirmDeleteDiscount}>{t('deleteConfirm')}</button>
          <button class="btn btn-sm btn-ghost" onClick={() => closeModal('delete-discount-modal')}>{t('cancel')}</button>
        </div>
      </Modal>

      {/* Delete batch confirmation */}
      <Modal id="delete-batch-modal" title={t('deleteConfirm')}>
        <p class="text-sm mb-4">{t('batches')}</p>
        <div class="modal-action">
          <button class="btn btn-error btn-sm" onClick={confirmDeleteBatch}>{t('deleteConfirm')}</button>
          <button class="btn btn-sm btn-ghost" onClick={() => closeModal('delete-batch-modal')}>{t('cancel')}</button>
        </div>
      </Modal>

      {/* Delete variant confirmation */}
      <Modal id="delete-variant-modal" title={t('deleteConfirm')}>
        <p class="text-sm mb-4">{t('variants')}</p>
        <div class="modal-action">
          <button class="btn btn-error btn-sm" onClick={confirmDeleteVariant}>{t('deleteConfirm')}</button>
          <button class="btn btn-sm btn-ghost" onClick={() => closeModal('delete-variant-modal')}>{t('cancel')}</button>
        </div>
      </Modal>
      ...
      <dialog id="loss-dialog" class="modal modal-bottom sm:modal-middle">
        <div class="modal-box">
          ...
          <p class="text-sm text-base-content/80 mb-3">{lossTarget?.name}</p>
          ...
          <form onSubmit={handleLoss}>
            <div class="flex gap-3 mb-3">
              <div class="flex flex-col flex-1">
                <span class="text-xs text-base-content/70 mb-0.5">{t('lossType')}</span>
              ...
              <div class="flex flex-col w-28">
                <span class="text-xs text-base-content/70 mb-0.5">{t('lossQty')}</span>
            ...
            <div class="flex flex-col mb-3">
              <span class="text-xs text-base-content/70 mb-0.5">{t('lossRemark')}</span>
      ...
      <dialog id="adj-dialog" class="modal modal-bottom sm:modal-middle">
        <div class="modal-box">
          ...
          <p class="text-sm text-base-content/80 mb-3">{adjTarget?.name}</p>
          <form onSubmit={handleAdjust}>
            <div class="flex gap-3 items-center mb-3">
              <div class="flex flex-col flex-1">
                <span class="text-xs text-base-content/70 mb-0.5">{t('qtyBefore')}</span>
                <input type="text" class="input input-bordered input-sm bg-base-200" value={adjTarget?.qty_available ?? 0} disabled />
              </div>
              <div class={`flex items-center pt-4 text-base-content/50 ${lang === 'ar' ? 'rotate-180' : ''}`}>
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" />
                </svg>
              </div>
              <div class="flex flex-col flex-1">
                <span class="text-xs text-base-content/70 mb-0.5">{t('qtyAfter')}</span>
            ...
            <div class="flex flex-col mb-3">
              <span class="text-xs text-base-content/70 mb-0.5">{t('reason')}</span>
            ...
            <div class="px-6 py-5 space-y-4">
              ...
              <div class="grid grid-cols-2 gap-3">
                <div class="rounded-xl border border-base-300 bg-base-200/40 p-4">
                  <div class="text-xs font-medium uppercase tracking-wider text-base-content/70 mb-1">{t('totalQty')}</div>
                  <div class="text-2xl font-bold tabular-nums">{Number(valuationData.total_qty || 0).toLocaleString()}</div>
                </div>
                <div class="rounded-xl border border-base-300 bg-base-200/40 p-4">
                  <div class="text-xs font-medium uppercase tracking-wider text-base-content/70 mb-1">{t('productCount')}</div>
      ...
      <dialog id="pricehist-dialog" class="modal modal-bottom sm:modal-middle">
        <div class="modal-box w-full sm:max-w-3xl">
          ...
          <p class="text-sm text-base-content/80 mb-3">{priceHistTarget?.name}</p>
          <div class="overflow-x-auto" style="max-height:350px; overflow-y:auto">
            <table class="table table-xs w-full">
              <thead class="sticky top-0 bg-base-200/60">
                <tr>
                  <th class="text-xs font-semibold uppercase tracking-wide text-base-content/70">{t('purchaseDate')}</th>
                  <th class="text-xs font-semibold uppercase tracking-wide text-base-content/70 text-end">{t('prixAchat')}</th>
                  {visiblePrices.pv1 && <th class="text-xs font-semibold uppercase tracking-wide text-base-content/70 text-end">{t('prixVente1')}</th>}
                  {visiblePrices.pv2 && <th class="text-xs font-semibold uppercase tracking-wide text-base-content/70 text-end">{t('prixVente2')}</th>}
                  {visiblePrices.pv3 && <th class="text-xs font-semibold uppercase tracking-wide text-base-content/70 text-end">{t('prixVente3')}</th>}
                  <th class="text-xs font-semibold uppercase tracking-wide text-base-content/70">{t('source')}</th>
                </tr>
              </thead>
              <tbody>
        ...
        <div class="modal-box p-0 overflow-hidden flex flex-col" style="width: 90vw; max-width: 900px; height: 85vh; max-height: 85vh">
          ...
          <div class="flex items-center justify-between px-6 py-4 border-b border-base-200 bg-base-100 shrink-0">
            <div>
              ...
              <p class="text-sm text-base-content/70">{variantTarget?.name}</p>
            </div>
            <div class="flex items-center gap-2">
              ...
              <form onSubmit={editingVariant ? saveEditVariant : addVariant} class="bg-base-200/40 rounded-xl p-4 mb-5 border border-base-200">
                ...
                <div class="mb-3">
                  <div class="flex items-center justify-between mb-1.5">
                    <span class="text-xs font-medium text-base-content/80">{t('attributes')}</span>
                  ...
                  <div class="space-y-1.5">
                    {variantForm.attrPairs.map((pair, i) => (
                      <div key={i} class="flex items-center gap-2">
                        <input class="input input-bordered input-sm flex-1" placeholder={t('attrKey')} value={pair.key}
                          onInput={(e) => updateAttrPair(i, 'key', e.target.value)} />
                        <span class="text-base-content/50">:</span>
                        <input class="input input-bordered input-sm flex-1" placeholder={t('attrValue')} value={pair.value}
                ...
                <label class="form-control mb-3">
                  <span class="label-text text-xs font-medium text-base-content/80 mb-1">{t('barcodes')}</span>
                  <input class="input input-bordered input-sm" placeholder={t('barcodes') + ' (' + t('commaSeparated') + ')'} value={variantForm.barcodes}
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-4">
                  <label class="form-control">
                    <span class="label-text text-xs font-medium text-base-content/80 mb-1">{t('qtyAvailable')}</span>
                  ...
                  <label class="form-control">
                    <span class="label-text text-xs font-medium text-base-content/80 mb-1">{t('prixAchat')}</span>
                  ...
                  {visiblePrices.pv1 && <label class="form-control">
                    <span class="label-text text-xs font-medium text-base-content/80 mb-1">{t('prixVente1')}</span>
                  ...
                  {visiblePrices.pv2 && <label class="form-control">
                    <span class="label-text text-xs font-medium text-base-content/80 mb-1">{t('prixVente2')}</span>
                  ...
                  {visiblePrices.pv3 && <label class="form-control">
                    <span class="label-text text-xs font-medium text-base-content/80 mb-1">{t('prixVente3')}</span>
                  ...
                  <div key={v.id} class={`rounded-xl border bg-base-100 p-4 transition-all hover:shadow-sm ${editingVariant === v.id ? 'border-primary/40 ring-1 ring-primary/20' : 'border-base-200'}`}>
                      ...
                      <div class="flex-1 min-w-0">
                        ...
                        {v.barcodes?.length > 0 && (
                          <div class="flex items-center gap-1.5 mb-2">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3 text-base-content/50 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 4.875c0-.621.504-1.125 1.125-1.125h4.5c.621 0 1.125.504 1.125 1.125v4.5c0 .621-.504 1.125-1.125 1.125h-4.5A1.125 1.125 0 0 1 3.75 9.375v-4.5zM3.75 14.625c0-.621.504-1.125 1.125-1.125h4.5c.621 0 1.125.504 1.125 1.125v4.5c0 .621-.504 1.125-1.125 1.125h-4.5a1.125 1.125 0 0 1-1.125-1.125v-4.5zM13.5 4.875c0-.621.504-1.125 1.125-1.125h4.5c.621 0 1.125.504 1.125 1.125v4.5c0 .621-.504 1.125-1.125 1.125h-4.5A1.125 1.125 0 0 1 13.5 9.375v-4.5z" /></svg>
                            <span class="text-xs font-mono text-base-content/70">{v.barcodes.join(', ')}</span>
                          </div>
                        )}
                        {/* Prices row */}
                      ...
                      <div class="flex items-center gap-1 shrink-0">
                        {canAdjust && (
                          <div class="tooltip tooltip-bottom" data-tip={t('adjustStock')}>
                            <button class="btn btn-sm btn-ghost btn-square text-base-content/70" onClick={() => openVariantAdj(v)}>
                    ...
                    {variantLossTarget?.id === v.id && (
                      <form onSubmit={submitVariantLoss} class="flex items-end gap-2 mt-2 pt-2 border-t border-base-200">
                        ...
                        <label class="form-control flex-1">
                          <span class="label-text text-xs">{t('remark')}</span>
      ...
      <dialog id="discount-dialog" class="modal modal-bottom sm:modal-middle">
        <div class="modal-box w-full sm:max-w-2xl p-0 overflow-hidden flex flex-col" style="max-height: 85vh">
          {/* Header */}
            ...
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p class="text-sm text-base-content/70">{discountTarget?.name}</p>
                </div>
              </div>
              <div class="flex items-center gap-2">
                {canEdit && (
                  <button type="button" class="btn btn-primary btn-sm gap-1.5" onClick={() => setDiscountFormOpen(!discountFormOpen)}>
                    {discountFormOpen
                      ? <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                      : <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>
                    }
                    {discountFormOpen ? t('cancel') : t('add')}
                  </button>
                )}
                <form method="dialog"><button class="btn btn-sm btn-ghost btn-square">
                  <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                </button></form>
              </div>
            </div>
          ...
          {canEdit && discountFormOpen && (
            <div class="px-6 py-4 border-b border-base-200 bg-primary/5 shrink-0">
              <form onSubmit={(e) => { addDiscount(e); setDiscountFormOpen(false) }}>
            ...
            {discountItems.length === 0 ? (
              <div class="text-center py-12">
                <div class="w-14 h-14 mx-auto rounded-full bg-base-200 flex items-center justify-center mb-3">
                  <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-base-content/50" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
                  </svg>
                </div>
                <p class="text-sm text-base-content/70">{t('noDiscountRules')}</p>
              </div>
            ) : (
              <div class="space-y-2">
                {discountItems.map(d => {
                    ...
                    <div key={d.id} class={`border rounded-xl p-3 transition-all ${isActive ? 'border-success/30 bg-success/5' : 'border-base-200 bg-base-200/30 opacity-60'}`}>
                      <div class="flex items-center justify-between gap-2">
                        <div class="flex items-center gap-3 flex-1 min-w-0">
                          <div class={`flex-shrink-0 px-3 py-1.5 rounded-lg font-bold font-mono text-sm ${isActive ? 'bg-success/15 text-success' : 'bg-base-300 text-base-content/70'}`}>
                          ...
                          <div class="min-w-0">
                            <div class="flex items-center gap-1.5 flex-wrap">
                              ...
                              {d.min_qty > 0 && <span class="badge badge-sm badge-ghost">{t('minQty')}: {d.min_qty}</span>}
                            ...
                            <div class="flex items-center gap-2 mt-1 text-xs text-base-content/70">
                              {d.start_date && <span>{t('dateFrom')}: {new Date(d.start_date).toLocaleDateString()}</span>}
                              {d.end_date && <span>{t('dateTo')}: {new Date(d.end_date).toLocaleDateString()}</span>}
                              {!d.start_date && !d.end_date && <span>—</span>}
                            </div>
                          </div>
                        </div>
                        {canEdit && (
                          <button class="btn btn-sm btn-ghost btn-square text-base-content/50 hover:text-error flex-shrink-0" onClick={() => deleteDiscount(d.id)}>
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
      ...
      <dialog id="batch-dialog" class="modal modal-bottom sm:modal-middle">
        <div class="modal-box w-full sm:max-w-2xl">
          ...
          <p class="text-sm text-base-content/80 mb-3">{batchTarget?.name}</p>

          <div class="overflow-x-auto mb-4" style="max-height:300px; overflow-y:auto">
            <table class="table table-xs">
              <thead class="sticky top-0 bg-base-100"><tr><th>{t('batchNumber')}</th><th>{t('expiryDate')}</th><th>{t('qty')}</th><th></th></tr></thead>
              <tbody>
                {batchItems.length === 0 && (
                  <tr><td colSpan={4} class="text-center text-base-content/70 py-4">{t('noBatches')}</td></tr>
                )}
                {batchItems.map(b => (
        ...
        <div class="modal-box p-0 overflow-hidden flex flex-col" style="width: 80vw; max-width: 80vw; height: 80vh; max-height: 80vh">
          ...
          <div class="bg-base-200/50 px-6 pt-5 pb-4 border-b border-base-300 shrink-0">
            <div class="flex items-start justify-between">
              <div class="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p class="text-sm text-base-content/70 mt-0.5">{movTarget?.name}</p>
                </div>
              </div>
              <form method="dialog">
                <button class="btn btn-sm btn-ghost btn-square opacity-60 hover:opacity-100">
            ...
            <div class="flex items-center gap-2 mt-4">
              <span class="text-xs font-medium text-base-content/70">{t('dateFrom')}</span>
              <input type="date" class="input input-bordered input-sm w-auto"
                value={movDateFrom} onInput={(e) => setMovDateFrom(e.target.value)} />
              <span class="text-xs font-medium text-base-content/70">{t('dateTo')}</span>
              <input type="date" class="input input-bordered input-sm w-auto"
            ...
            {movLoading ? (
              ...
              <table class="table table-sm w-full">
                <thead class="sticky top-0 bg-base-100 z-10">
                  <tr>
                    <th class="text-xs font-semibold uppercase tracking-wide text-base-content/70">{t('purchaseDate')}</th>
                    <th class="text-xs font-semibold uppercase tracking-wide text-base-content/70">{t('movementType')}</th>
                    <th class="text-xs font-semibold uppercase tracking-wide text-base-content/70">{t('purchaseSupplier')}</th>
                    <th class="text-xs font-semibold uppercase tracking-wide text-base-content/70 text-end">{t('qty')}</th>
                    <th class="text-xs font-semibold uppercase tracking-wide text-base-content/70 text-end">{t('prixAchat')}</th>
                    <th class="text-xs font-semibold uppercase tracking-wide text-base-content/70 text-end">Total</th>
                  </tr>
                </thead>
                <tbody>
                  {movAllItems.length === 0 && (
                    <tr>
                      <td colSpan={6} class="text-center text-base-content/50 py-16">
                    ...
                    <tr key={i} class="hover:bg-base-200/40 transition-colors">
                      ...
                      <td>
                        <div class="flex flex-col gap-0.5">
                          ...
                          {m.variant_label && <span class="text-xs text-primary/60">{m.variant_label}</span>}
                        </div>
                      </td>
                      <td class="text-sm text-base-content/80">{m.supplier_name || '—'}</td>
          ...
          {movPages > 1 && (
            <div class="flex items-center justify-between px-6 py-3 border-t border-base-300 shrink-0 bg-base-200/30">
              <span class="text-xs text-base-content/70 tabular-nums">{movAllItems.length} {t('results')}</span>
              <div class="join">
      ...
      {/* Action menu portal — rendered at root level to escape overflow:auto */}
      {actionMenu && (
        <>
          <div class="fixed inset-0 z-[9998]" onClick={() => setActionMenu(null)} />
          <ul class="menu menu-sm bg-base-100 rounded-lg shadow-xl border border-base-200 w-48 z-[9999] p-1"
            style={`position:fixed; ${actionMenu.above ? `bottom:${window.innerHeight - actionMenu.y}px` : `top:${actionMenu.y}px`}; left:${Math.max(8, actionMenu.x - 192)}px;`}>
            {canLoss && !actionMenu.product.has_variants && (
              <li><a onClick={() => { setActionMenu(null); openLoss(actionMenu.product) }} class="text-warning text-xs gap-2">
            ...
            {canMovement && (
              <li><a onClick={() => { setActionMenu(null); openMovements(actionMenu.product) }} class="text-xs gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z" /></svg>
                {t('stockMovements')}
              </a></li>
            )}
            {canAdjust && !actionMenu.product.is_service && !actionMenu.product.has_variants && (
              <li><a onClick={() => { setActionMenu(null); openAdjust(actionMenu.product) }} class="text-xs gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M10.5 6h9.75M10.5 6a1.5 1.5 0 1 1-3 0m3 0a1.5 1.5 0 1 0-3 0M3.75 6H7.5m3 12h9.75m-9.75 0a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m-3.75 0H7.5m9-6h3.75m-3.75 0a1.5 1.5 0 0 1-3 0m3 0a1.5 1.5 0 0 0-3 0m-9.75 0h9.75" /></svg>
                {t('adjustStock')}
              </a></li>
            )}
            {canPriceHist && (
              <li><a onClick={() => { setActionMenu(null); openPriceHistory(actionMenu.product) }} class="text-xs gap-2">
            ...
            {canAdd && (
              <li><a onClick={() => { setActionMenu(null); handleDuplicate(actionMenu.product) }} class="text-xs gap-2">
            ...
            {canVariants && (
              <li><a onClick={() => { setActionMenu(null); openVariants(actionMenu.product) }} class="text-xs gap-2">
            ...
            {canDiscounts && (
              <li><a onClick={() => { setActionMenu(null); openDiscounts(actionMenu.product) }} class="text-xs gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5"><path stroke-linecap="round" stroke-linejoin="round" d="M9 14.25l6-6m4.5-3.493V21.75l-3.75-1.5-3.75 1.5-3.75-1.5-3.75 1.5V4.757c0-1.108.806-2.057 1.907-2.185a48.507 48.507 0 0 1 11.186 0c1.1.128 1.907 1.077 1.907 2.185zM9.75 9h.008v.008H9.75V9zm4.5 5.25h.008v.008h-.008v-.008z" /></svg>
                {t('discountRules')}
              </a></li>
            )}
            {canBatches && !actionMenu.product.is_service && (
              <li><a onClick={() => { setActionMenu(null); openBatches(actionMenu.product) }} class="text-xs gap-2">
            ...
            {canArchive && (
              <li><a onClick={() => { setActionMenu(null); handleArchive(actionMenu.product) }} class="text-warning text-xs gap-2">
