...
export default function Brands({ path }) {
  ...
  function closeAllDialogs() {
    closeModal('brand-modal'); closeModal('brand-delete-modal')
    setDeleteTarget(null)
  }

  function openCreate() {
    closeAllDialogs(); setEditing(null); setForm({ name: '' }); setError(''); openModal('brand-modal')
  }

  function openEdit(item) {
    closeAllDialogs(); setEditing(item); setForm({ name: item.name }); setError(''); openModal('brand-modal')
  }
                      ...
                      <div class="flex gap-1">
                        ...
                        {canDelete && (
                          <div class="tooltip tooltip-left" data-tip={t('delete')}>
                            <button class="btn btn-sm btn-ghost btn-square text-error" onClick={() => { closeAllDialogs(); setDeleteTarget(item); openModal('brand-delete-modal') }}>
                              <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
