...
export default function Categories({ path }) {
  const { t } = useI18n()
  const canAdd    = hasPerm('categories', 'add')
  ...
  const load = useCallback(async () => {
    try {
      const data = await api.listCategoriesPage({ q: searchQ, page, limit: 10 })
      setResult(data)
    } catch {}
  }, [searchQ, page])
  ...
  useEffect(() => {
    let cancelled = false
    api.listCategoriesPage({ q: searchQ, page, limit: 10 })
  ...
  function openCreate() {
    setEditing(null); setForm({ name: '' }); setError(''); openModal('cat-modal')
  }

  function openEdit(item) {
    setEditing(item); setForm({ name: item.name }); setError(''); openModal('cat-modal')
  }
  ...
  async function confirmDelete() {
    ...
    try { await api.deleteCategory(deleteTarget.id); setDeleteTarget(null); closeModal('cat-delete-modal'); load() } catch {}
  }
  ...
  return (
    <Layout currentPath={path}>
      <div class="flex items-center justify-between mb-6">
        <h2 class="text-2xl font-bold">{t('categoriesPage')}</h2>
        {canAdd && <button class="btn btn-primary btn-sm" onClick={openCreate}>{t('newCategory')}</button>}
      </div>

      <div class="bg-base-100 rounded-xl shadow-sm border border-base-300 p-3 mb-4 flex gap-2">
        <input class="input input-bordered input-sm flex-1 max-w-xs" placeholder={t('search')}
        ...
        <div class="overflow-x-auto">
          <table class="table table-sm w-full">
            <thead class="bg-base-200/60">
              <tr>
                <th class="px-3 py-2.5 text-xs font-semibold uppercase tracking-wide text-base-content/50">{t('categoryName')}</th>
                {canWrite && <th class="px-3 py-2.5 text-xs font-semibold uppercase tracking-wide text-base-content/50 w-24">{t('actions')}</th>}
              </tr>
            </thead>
            <tbody>
              {items.map((item) => (
                      ...
                      <div class="flex gap-1">
                        {canEdit && (
                          <div class="tooltip tooltip-left" data-tip={t('edit')}>
                            <button class="btn btn-xs btn-ghost btn-square" onClick={() => openEdit(item)}>
                        ...
                        {canDelete && (
                          <div class="tooltip tooltip-left" data-tip={t('delete')}>
                            <button class="btn btn-xs btn-ghost btn-square text-error" onClick={() => { setDeleteTarget(item); openModal('cat-delete-modal') }}>
                              <svg xmlns="http://www.w3.org/2000/svg" class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                  ...
                  <td colSpan={canWrite ? 2 : 1} class="px-3 py-12 text-center">
                    <div class="flex flex-col items-center gap-2 text-base-content/30">
                      <svg xmlns="http://www.w3.org/2000/svg" class="w-10 h-10" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9.568 3H5.25A2.25 2.25 0 003 5.25v4.318c0 .597.237 1.17.659 1.591l9.581 9.581c.699.699 1.78.872 2.607.33a18.095 18.095 0 005.223-5.223c.542-.827.369-1.908-.33-2.607L9.568 3zM6 6h.008v.008H6V6z" />
                      </svg>
                      <p class="text-sm">{t('noCategories')}</p>
                    </div>
                  </td>
                </tr>
              )}
      ...
      <Modal id="cat-modal" title={editing ? t('editCategory') : t('newCategory')}>
        {error && <div class="alert alert-error text-sm py-2 mb-3"><span>{error}</span></div>}
        <form onSubmit={handleSubmit} class="space-y-3">
          <label class="form-control">
            <span class="label-text text-sm">{t('categoryName')}</span>
          ...
          <div class="modal-action">
            <button type="submit" class={`btn btn-primary btn-sm ${loading ? 'loading' : ''}`} disabled={loading}>
              {editing ? t('saveChanges') : t('newCategory')}
            </button>
          </div>
        </form>
      </Modal>

      <Modal id="cat-delete-modal" title={t('deleteCategory')}>
        <p class="text-sm mb-4">{deleteTarget?.name}</p>
        <div class="modal-action">
          <button class="btn btn-error btn-sm" onClick={confirmDelete}>{t('delete')}</button>
          <button class="btn btn-ghost btn-sm" onClick={() => closeModal('cat-delete-modal')}>{t('back')}</button>
        </div>
      </Modal>
    </Layout>
  )
