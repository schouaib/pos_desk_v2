      ...
      <div class="card bg-base-100 shadow overflow-hidden">
        ...
        <table class="table table-sm w-full">
          <thead class="bg-base-200/60">
            <tr>
              <th class="px-3 py-2.5 text-xs font-semibold uppercase tracking-wide text-base-content/70 whitespace-nowrap">{t('expenseDateFrom')}</th>
              <th class="px-3 py-2.5 text-xs font-semibold uppercase tracking-wide text-base-content/70 whitespace-nowrap">{t('expenseDateTo')}</th>
              <th class="px-3 py-2.5 text-xs font-semibold uppercase tracking-wide text-base-content/70 text-center">{t('expenseDays')}</th>
              <th class="px-3 py-2.5 text-xs font-semibold uppercase tracking-wide text-base-content/70">{t('expenseLabel')}</th>
              <th class="px-3 py-2.5 text-xs font-semibold uppercase tracking-wide text-base-content/70 text-end">{t('expenseAmount')}</th>
              <th class="px-3 py-2.5 text-xs font-semibold uppercase tracking-wide text-base-content/70 text-end">{t('expenseDailyAmt')}</th>
              <th class="px-3 py-2.5 text-xs font-semibold uppercase tracking-wide text-base-content/70">{t('expenseNote')}</th>
          ...
          <tbody>
            ...
            {!loading && items.length === 0 && (
              <tr><td colSpan={8} class="px-3 py-12 text-center text-base-content/50 text-sm">{t('noExpenses')}</td></tr>
            )}
              ...
              <tr key={exp.id} class="border-b border-base-200 hover:bg-base-50 transition-colors">
                ...
                <td class="px-3 py-2.5 text-end">
                  <div class="flex items-center justify-end gap-1">
                    {canEdit && <button class="btn btn-sm btn-ghost btn-square" onClick={() => openEdit(exp)}>
                      <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L6.832 19.82a4.5 4.5 0 01-1.897 1.13l-2.685.8.8-2.685a4.5 4.5 0 011.13-1.897L16.863 4.487zm0 0L19.5 7.125" />
                      </svg>
                    </button>}
                    {canDelete && <button class="btn btn-sm btn-ghost btn-square text-error" onClick={() => setDeleteTarget(exp)}>
                      <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
